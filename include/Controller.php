	        ...
	        private function registration_patient()
	        {
	         ...
	         private function registration_hospital()
	         {
	             if($_SERVER['REQUEST_METHOD']=="POST")
	             {
                     ...
                     header("location:../../medical/hospital_account.php");
	
	             }
	
	         }
        private function login_hospital()
        {
            if($_SERVER['REQUEST_METHOD']=="POST")
            {
                $this->userdata[0] = $_POST['username'];
                $this->userdata[1] = $_POST['password'];
                $datauser = $this->model_obj->login_hospital($this->userdata);
	        ...
	        private function registration_doctor()
	        {
	            ...
	            if($_SERVER['REQUEST_METHOD']=="POST")
	            {
	                ...
	                $this->userdata[1]= $this->generate_username_password(10);
                    ...
                    header("location:../../medical/manage_doc.php");
	        ...
	        if($_SERVER['REQUEST_METHOD']=="POST")
	            {
				    ...
				    $this->userdata[1]= $_POST['password'];
				    $this->model_obj->login_doctor($this->userdata);
                    header("location:../../mtcrs/doctor_account.php");
	               ...
	               switch(true)
	                {
                       ...
                       case isset($_POST['date']):
	                       ...
	                       break;
                        case isset($_POST['del_mobile_no']):
                            $this->model_obj->delete_dr($_POST['del_mobile_no']);
                            echo json_encode("success");
                            break;
                       case isset($_POST['dr_list']):
                           echo json_encode(($this->model_obj->doctor_list()));
                           break;
	                    default: echo json_encode("false from api_request");
	                        break;
	    ...
	    public function create_uri($par)
	    {
	        ...
	        switch($par)
	        {
	            ...
	            case 'cu':
	                ...
	                break;
                case 'lh':
                    $obj = new Main_controller('login_hospital');
                    break;
	            default:
